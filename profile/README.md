...
GDI operates on a federated model consisting of central (European-level) and local (node-level) components. Individual national nodes deploy and manage their own environments using the Starter Kit.
...
| [FAIR Data Point CKAN extension](https://github.com/GenomicDataInfrastructure/gdi-userportal-ckanext-fairdatapoint) | The CKAN extension implementation of the FAIR Data Point protocol, that addes new harvesting endpoints to CKAN. |
